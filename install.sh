...
if [[ $webserver =~ ^[Yy]$ ]]; then
    ...
    sudo apt-get update
    PHP_VER="8.2"
    sudo apt-get install -y apache2 php$PHP_VER php$PHP_VER-curl php$PHP_VER-mbstring php$PHP_VER-xml php$PHP_VER-gd php$PHP_VER-dev php-pear php$PHP_VER-ssh2 libmcrypt-dev mcrypt make php$PHP_VER-json php$PHP_VER-bcmath php$PHP_VER-intl php$PHP_VER-mysql php$PHP_VER-ldap php$PHP_VER-zip php$PHP_VER-soap php$PHP_VER-imap php$PHP_VER-memcached php$PHP_VER-redis php$PHP_VER-apcu
...
2)
    ...
    sudo cp $HOME/Phpnuxbill-FreeRADIUS/config-files/default $freeradius_config_dir/sites-available/default
    sudo mv $freeradius_config_dir/sites-available/inner-tunnel $freeradius_config_dir/sites-available/inner-tunnel.back
    sudo cp $HOME/Phpnuxbill-FreeRADIUS/config-files/inner-tunnel $freeradius_config_dir/sites-available/inner-tunnel
    sudo mv $freeradius_config_dir/mods-available/sql $freeradius_config_dir/mods-available/sql.back
    sudo cp $HOME/Phpnuxbill-FreeRADIUS/config-files/sql $freeradius_config_dir/mods-available/sql
    sudo mv $freeradius_config_dir/mods-available/sqlcounter $freeradius_config_dir/mods-available/sqlcounter.back
    sudo cp $HOME/Phpnuxbill-FreeRADIUS/config-files/sqlcounter $freeradius_config_dir/mods-available/sqlcounter
    sudo cp $HOME/Phpnuxbill-FreeRADIUS/config-files/radiusd.conf $freeradius_config_dir/radiusd.conf
    sudo cp -r $HOME/Phpnuxbill-FreeRADIUS/config-files/mysql/* $freeradius_config_dir/mods-config/sql/counter/mysql/
    ...
    ;;
3)
    # Install PHPNuxBill + FreeRADIUS
    echo "Installing PHPNuxBill + FreeRADIUS..."
    sleep 2
    git clone https://github.com/hotspotbilling/phpnuxbill.git
    sudo mv phpnuxbill /var/www/html/
    sudo chown -R www-data:www-data /var/www/html/phpnuxbill
...
#                                                                  #
#  You can now access PHPNuxBill at: http://your-server-ip/        #
#                                                                  #
####################################################################
"
